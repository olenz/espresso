...
explicitly specified using the feature \feature{FLATNOISE}. You can also
employ Gaussian noise which is, in some sense, more realistic. Notably
Gaussian noise (activated using the feature \feature{GAUSSRANDOM}) does
...
feature \feature{GAUSSRANDOMCUT}, which produces Gaussian random numbers
...
\subsection{Langevin thermostat: z only}
\begin{essyntax}
  thermostat langevin\_z\_only \var{on/off}
\end{essyntax}

If the Langevin thermostat is already set, is is possible by activating this 
switch to apply thermostatting only to movement in the z direction of the simulation box.

The purpose of this is as a crude measure to thermostat systems which have some 
flow in the xy plane but no symmetry-breaking in z.

Treatment rotational degrees of freedom is not altered by this switch.
