  ...
  if(ARG0_IS_S("tolerance_field")) {
    ...
    if(! (argc > 1 && ARG1_IS_I(scafacos.short_range_flag) && (scafacos.short_range_flag ==1 || scafacos.short_range_flag == 0))) {
      ...
      if(! (argc > 1 && ARG1_IS_I(scafacos.virial) && (scafacos.virial ==1 || scafacos.virial == 0))) {
        Tcl_AppendResult(interp, "viriral expects 0 or 1",	 (char *) NULL);
        return TCL_ERROR;
      }
    } 
...
int tclcommand_inter_coulomb_parse_scafacos_p3m(Tcl_Interp * interp, int argc, char ** argv)
{
  ...
  while(argc > 0) {
    if (ARG0_IS_S("tune"))
      return tclcommand_inter_coulomb_parse_scafacos_p3m_conventional_tune(interp, argc-1, argv+1);

    if (ARG0_IS_S("tunev2"))
      return tclcommand_inter_coulomb_parse_scafacos_p3m_tune(interp, argc-1, argv+1, 1);
