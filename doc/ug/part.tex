...
\item[\opt{torque \var{x} \var{y} \var{z}}] Sets the rotational force
  ...
  have exactly the same conformation. Again, any torque set this way
  is lost during the next integration step.
...
\begin{essyntax}
  ...
  \optlong{\alt{id \asep pos \asep type \asep folded_position \asep type \asep
      q \asep v \asep f \asep fix \asep ext_force \asep ext_torque \asep bond \asep
...
By default the particle position returned is {\bf unfolded}, for convenience in diffusion calculations etc. If \feature{LEES_EDWARDS} is turned on then particle positions are output with whatever degree of folding or unfolding is present in the system state at that time, to try and avoid the common error that imaged positions and un-imaged velocities (or vice-versa) are saved together.  If the folded position is requested explicitly in a Lees-Edwards simulation, then the folded velocities will also be returned.
